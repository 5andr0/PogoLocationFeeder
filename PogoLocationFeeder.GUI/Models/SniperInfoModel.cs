...
namespace PogoLocationFeeder.GUI.Models { 
    ...
    public class SniperInfoModel
    {
        ...
        public void CopyCoords()
        {
            try {
                Clipboard.SetText(
                    $"{Info.Latitude.ToString("N6", CultureInfo.InvariantCulture)}, {Info.Longitude.ToString("N6", CultureInfo.InvariantCulture)}");
        ...
        private void StartProcessWithPath() {
            try
            {
                ...
                process.StartInfo.Arguments =
                    $"pokesniper2://{Info.Id}/{Info.Latitude.ToString("N6", CultureInfo.InvariantCulture)},{Info.Longitude.ToString("N6", CultureInfo.InvariantCulture)}";
                process.Start();
        ...
        public void PokeSnipers()
        {
            try
            {
                if (GlobalSettings.PokeSnipers2Exe.Contains(".exe")) {
                    ...
                    var process = Process.Start($"pokesniper2://{Info.Id}/{Info.Latitude.ToString("N6", CultureInfo.InvariantCulture)},{Info.Longitude.ToString("N6", CultureInfo.InvariantCulture)}");
                    KillProcessLater(process);
                }
            }
            catch (Exception e)
